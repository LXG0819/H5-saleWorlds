...
export default {
  ...
  /**
   * 录制视频
   * @param time { Number } 录制时间 默认为 20s
   * 
   */
  videoRecord (time = 20) {
    return this.callApi(_MIDEA_COMMON, 'videoRecord', [time])
  },
  /**
   * 文件上传
   * data { Array } 上传路径、文件路径
   *   如： [{
        url: '上传地址路径',
        files: [文件路径, 文件路径]
      }]
   */
  uploadFile (data = []) {
    return this.callApi('MCFileTransfer', 'upload', data)
  },
  /**
   *  开始录音
   *  time 最大录音时间，单位：毫秒 (默认为15s)
   */
  playAudio(time = 15000) {
    return this.callApi('MCRecorder', 'startRecord', [time])
  },
  /**
   *  停止录音 
   */
  stopAudio() {
    return this.callApi('MCRecorder', 'stopRecord', [])
  },
  /**
   * 取消录音
   */
  cancelRecord() {
    return this.callApi('MCRecorder', 'cancelRecord', [])
  },
  /**
   * 保存到相册
   */
  saveToGallery(url) {
    return this.callApi('MideaCommon', 'saveToGallery', [url])
  }
}
